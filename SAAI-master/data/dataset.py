...
class SYSUDataset(Dataset):
    def __init__(self, root, mode='train', transform=None):
        ...
        img_paths = [path for path in img_paths if int(path.split('/')[-2]) in selected_ids] #linux
        # img_paths = [path for path in img_paths if int(path.split('\\')[-2]) in selected_ids] #windows
