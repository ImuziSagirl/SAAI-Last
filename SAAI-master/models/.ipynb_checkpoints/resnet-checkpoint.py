...
from layers.module.my_module import ECAAttention, SEAttention,SplitAttn,AttentionModule,EMA
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, zero_init_residual=False, modality_attention=0,
        ...
        # self.MSCA1 = AttentionModule(dim=256)
        # self.MSCA2 = AttentionModule(dim=512)
        # self.MSCA3 = AttentionModule(dim=1024)
        # self.MSCA4 = AttentionModule(dim=2048)
    ...
    def forward(self, x):
        ...
        x = self.layer1(x)
        x = self.ECA1(x)  # my code
        #x = self.SplitAttn1(x)
        #x = self.EMA1(x)
        x = self.layer2(x)
        x = self.ECA2(x)  # my code
        #x = self.SplitAttn2(x)
        #x = self.EMA2(x)
        x = self.layer3(x)
        x = self.ECA3(x)   # my code
        #x = self.SplitAttn3(x)
        #x = self.EMA3(x)
        if self.modality_attention > 1:
            x = self.MAM3(x)
